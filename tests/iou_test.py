...
class TestIntersectionOverUnion(unittest.TestCase):
    ...
    def test_both_inside_cell_shares_area(self):
        ious = iou(self.t1_box1, self.t1_box2, format="midpoint")
        self.assertTrue((torch.abs(ious - self.t1_correct_iou) < self.epsilon))

    def test_partially_outside_cell_shares_area(self):
        ious = iou(self.t2_box1, self.t2_box2, format="midpoint")
        self.assertTrue((torch.abs(ious - self.t2_correct_iou) < self.epsilon))

    def test_both_inside_cell_shares_no_area(self):
        ious = iou(self.t3_box1, self.t3_box2, format="midpoint")
        self.assertTrue((torch.abs(ious - self.t3_correct_iou) < self.epsilon))

    def test_midpoint_outside_cell_shares_area(self):
        ious = iou(self.t4_box1, self.t4_box2, format="midpoint")
        self.assertTrue((torch.abs(ious - self.t4_correct_iou) < self.epsilon))

    def test_both_inside_cell_shares_entire_area(self):
        ious = iou(self.t5_box1, self.t5_box2, format="midpoint")
        self.assertTrue((torch.abs(ious - self.t5_correct_iou) < self.epsilon))

    def test_box_format_x1_y1_x2_y2(self):
        ious = iou(self.t6_box1, self.t6_box2, format="corners")
        self.assertTrue((torch.abs(ious - self.t6_correct_iou) < self.epsilon))

    def test_additional_and_batch(self):
        ious = iou(
            self.t12_bboxes1, self.t12_bboxes2, format="corners"
        )
        all_true = torch.all(
